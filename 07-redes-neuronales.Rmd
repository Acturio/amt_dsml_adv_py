...
activation_fig.add_hline(y=0, line_width=1, line_color="#9ca3af")
activation_fig.add_vline(x=0, line_width=1, line_color="#9ca3af")
activation_fig.update_traces(
    hovertemplate="Entrada: %{x:.2f}<br>Salida: %{y:.3f}<extra></extra>"
)
print(interactive_html(activation_fig))
```
...
softmax_fig = px.bar(
    ...
    y="probabilidad",
    text="probabilidad",
    title="Softmax: probabilidades a partir de logits",
)
softmax_fig.update_traces(
    marker_color="#0f766e",
    texttemplate="%{text:.3f}",
    hovertemplate="Clase %{x}<br>Probabilidad: %{y:.3f}<extra></extra>",
)
softmax_fig.update_yaxes(range=[0, 1])
print(interactive_html(softmax_fig))
```
...
loss_fig = px.line(
    ...
    y="valor",
    color="perdida",
    title="Comparación de pérdidas para regresión",
)
loss_fig.update_yaxes(title="Pérdida", range=[0, 8])
loss_fig.update_xaxes(title="Error = predicción - observación")
loss_fig.update_traces(
    hovertemplate="Error: %{x:.2f}<br>Pérdida: %{y:.3f}<extra></extra>"
)
print(interactive_html(loss_fig))
```
...
En la animación siguiente, el punto parte de un parámetro con pérdida alta y actualiza su posición con el gradiente de la curva. La tasa de aprendizaje controla qué tan largo es cada paso.
...
gd_history_fig = px.line(
    gd_history_df,
    x="epoca",
    y="mse",
    color="tipo",
    log_y=True,
    title="Batch vs estocástico vs mini-batch",
)
gd_history_fig.update_xaxes(title="Época")
gd_history_fig.update_yaxes(title="MSE (escala logarítmica)")
gd_history_fig.update_traces(
    hovertemplate="Época: %{x}<br>MSE: %{y:.4f}<extra></extra>"
)
print(interactive_html(gd_history_fig))
```
...
optimizer_fig = px.line(
    optimizer_history_df,
    x="epoca",
    y="mse",
    color="optimizador",
    log_y=True,
    title="Comparación de optimizadores en un problema simple",
)
optimizer_fig.update_xaxes(title="Época")
optimizer_fig.update_yaxes(title="MSE (escala logarítmica)")
optimizer_fig.update_traces(
    hovertemplate="Época: %{x}<br>MSE: %{y:.4f}<extra></extra>"
)
print(interactive_html(optimizer_fig))
```
...
regularization_fig = px.line(
    comparison_long,
    x="epoch",
    y="mse",
    color="modelo",
    facet_col="conjunto",
    title="Contraste de pérdida: sin regularización y regularizada",
)
regularization_fig.update_xaxes(title="Época")
regularization_fig.update_yaxes(title="MSE estandarizado")
regularization_fig.for_each_annotation(
    lambda annotation: annotation.update(text=annotation.text.split("=")[-1])
)
regularization_fig.update_traces(
    hovertemplate="Época: %{x}<br>MSE: %{y:.4f}<extra></extra>"
)
print(interactive_html(regularization_fig))
```
